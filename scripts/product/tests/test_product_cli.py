...
class ProductInboxWorkflowStaticTest(unittest.TestCase):
    ...
    def test_bug_automation_workflow_has_bounded_repair(self):
        ...
        self.assertIn("should_reconcile_success", text)
        self.assertIn("listPullRequestsAssociatedWithCommit", text)
        self.assertIn("Reconcile merged bug validation after successful master CI", text)
        self.assertIn("--implementation-issue-number", text)
        self.assertIn("--merged true", text)
        self.assertIn("--merged-by", text)
